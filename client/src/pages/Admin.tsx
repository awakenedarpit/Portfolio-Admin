...
export default function Admin() {
  ...
  const [newProject, setNewProject] = useState({ title: "", category: "", description: "", technologies: "", github: "", live: "", accent: "violet", project_index: "" });
  const dashboardSections = [
    ["overview", "Overview", "Workspace pulse", "LIVE"], ["identity", "Identity & About", "Hero, about, focus", "LIVE"],
    ["skills", "Skills", "Tools & learning", "LIVE"], ["projects", "Projects", "Work & galleries", "LIVE"],
    ["hackathons", "Hackathons", "Showcase stories", "LIVE"], ["journal", "Journal", "Notes & writing", "LIVE"], ["journey", "Journey", "Timeline entries", "LIVE"],
    ["certifications", "Certifications", "Proof & links", "LIVE"], ["contact", "Contact & Links", "Social destinations", "LIVE"],
  ];
  ...
  async function createProject(event: React.FormEvent) {
    ...
    if (!title) { setMessage("Add a project name first."); return; }
    const nextIndex = newProject.project_index.trim() || String(items.length + 1).padStart(2, "0");
    const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
    setMessage(`Creating ${title}…`);
    const { error } = await supabase.from("portfolio_projects").insert({ slug, title, category: newProject.category, description: newProject.description, technologies: newProject.technologies.split(",").map((item) => item.trim()).filter(Boolean), github: newProject.github, live: newProject.live, accent: newProject.accent, project_index: nextIndex, featured: false, thumbnail_url: "" });
    if (error) { setMessage(error.message); return; }
    setNewProject({ title: "", category: "", description: "", technologies: "", github: "", live: "", accent: "violet", project_index: "" });
    setCreateProjectOpen(false);
    await loadProjects();
    setMessage(`${title} added to Selected experiments.`);
  }
  ...
  function openEditor(id: string, label: string, status: string) {
    ...
    const targetId = id === "projects" ? "projects-editor" : id === "hackathons" ? "showcase-editor" : id === "journal" ? "journal-editor" : id;
    const target = ["projects", "hackathons", "journal"].includes(id) ? document.getElementById(targetId) : document.querySelector(`#site-content #${targetId}`);
  ...
  async function deleteProject(index: number) {
    ...
    if (!item.id || !window.confirm(`Delete ${item.title} from Selected experiments? This cannot be undone.`)) return;
    setMessage(`Deleting ${item.title}…`);
    const images = await supabase.from("portfolio_project_images").delete().eq("project_id", item.id);
    if (images.error) { setMessage(images.error.message); return; }
    const { error } = await supabase.from("portfolio_projects").delete().eq("id", item.id);
    if (error) { setMessage(error.message); return; }
    setItems((current) => current.filter((_, itemIndex) => itemIndex !== index));
    setMessage(`${item.title} deleted from the public portfolio.`);
  }
